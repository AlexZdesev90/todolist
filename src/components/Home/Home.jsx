...
export class Home extends Component {
    ...
    onChangeSearchValue = (value) => {
        const searchedItems = this.state.init.filter((item) => item.title.toLowerCase().includes(value.toLowerCase()));
    ...
    onClickDoneHandlier = (id) => {
        const newItems = this.state.items.map(item => item = item.id === id ? {...item, done: !item.done} : item)
    ...
    onClickAddInArchive = (id) => {
        const newItem = this.state.items.map(item => item = item.id === id ? { ...item, archive: !item.archive} : item )
        // console.log(newItem)
    ...
    render () {
        ...
        return (
            <div className={[classes.base, `${darkTheme ? classes.darkhome : classes.home}`].join(" ")}>
                ...
                <Time/>
                {showArchive ? <Archive archive={archive} items={items} onClickArchiveClose={this.onClickArchiveClose}/> : null}
                {modal ? <CreateTodo onClickExit={this.onClickExit} items={items} setItems={this.addItems}/> : null}
                <Box idHandlier={this.idHandlier}/>
                <span className={classes.lengthInfo}>You have {init.length} todos</span><br/>
                <Search items={items} onChangeSearchValue={this.onChangeSearchValue} onClickDelete={this.onClickDelete}/>
                {items.length > 0 ? <Items archive={archive} changeValue={this.changeValue} items={items} removeTodo={this.deleteTodo} onClickDoneHandlier={this.onClickDoneHandlier} onSetItems={this.setState} onClickAddInArchive={this.onClickAddInArchive}/> 
                : <div className={classes.info}>Application is clear. You  need open menu &nbsp;<CiMenuBurger/>&nbsp; and create todo !</div>}
            </div>
        )
    }
}
